<x-app-layout>
    ...
    <div class="py-12 bg-slate-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- ALERT NOTIFIKASI --}}
            @if (session('success'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-[2rem] font-bold text-sm flex items-center gap-4 alert-notif">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($exams as $exam)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-8 py-6">
                                    <span class="px-3 py-1 {{ $exam->status === 'open' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400' }} rounded-full text-[9px] font-black uppercase">
                                        {{ $exam->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-6 font-black text-slate-800 uppercase text-sm">{{ $exam->name }}</td>
                                ...
                                <td class="px-6 py-6 text-[10px] font-bold text-slate-400 uppercase">{{ $exam->location }}</td>
                                <td class="px-6 py-6 text-center font-black text-slate-700">{{ $exam->participants->count() }}</td>
                                <td class="px-8 py-6">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        @if (auth()->user()->hasRole(['pb', 'pengprov']))
                                            ...
                                            <form action="{{ route('admin.exams.destroy', $exam->id) }}" method="POST" onsubmit="return confirm('Hapus jadwal ini?')">
                                                @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-20 text-center font-bold text-slate-400 uppercase tracking-widest text-xs">Belum ada data</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                @foreach ($exams as $exam)
                    <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                        <div class="flex justify-between mb-4">
                            <span class="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-black uppercase">{{ $exam->status }}</span>
                        ...
                        <div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl">
                            <span class="text-[10px] font-black text-slate-400 uppercase">{{ $exam->participants->count() }} Peserta</span>
    ...
    {{-- MODAL TAMBAH --}}
    <div id="modalAddExam" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl border border-slate-100">
            <form action="{{ route('admin.exams.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <input type="text" name="name" placeholder="Nama Sesi" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold">
                    <div class="grid grid-cols-2 gap-4">
                        <input type="date" name="execution_date" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold">
                        <select name="status" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold">
                            <option value="open">Open</option>
                            <option value="draft">Draft</option>
                        </select>
                    </div>
                    <input type="text" name="location" placeholder="Lokasi" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold">
                </div>
                <div class="mt-8 flex gap-3">
                    <button type="button" onclick="toggleModal('modalAddExam')" class="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-[10px] uppercase">Batal</button>
                    <button type="submit" class="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- MODAL EDIT --}}
    <div id="modalEditExam" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl border border-slate-100">
            <form id="editForm" method="POST">
                @csrf @method('PUT')
                <h3 class="text-2xl font-black text-slate-800 uppercase mb-6">Edit Jadwal</h3>
                <div class="space-y-4">
                    <input type="text" name="name" id="edit_name" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold">
                    <div class="grid grid-cols-2 gap-4">
                ...
                <div class="mt-8 flex gap-3">
                    <button type="button" onclick="toggleModal('modalEditExam')" class="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-[10px] uppercase">Batal</button>
                    <button type="submit" class="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">Update Jadwal</button>
                </div>
            </form>
        </div>
    </div>
