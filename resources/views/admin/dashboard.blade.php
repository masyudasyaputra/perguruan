<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-4 sm:px-0">
            ...
            {{-- Badge Role Akses --}}
            <div
                class="flex items-center gap-3 bg-slate-900/50 backdrop-blur-xl p-2 pr-5 rounded-2xl shadow-xl border border-slate-800">
    ...
    {{-- Penyesuaian Background Utama --}}
    <div class="py-8 min-h-screen relative overflow-hidden">


        <div class="max-w-7xl justify-between mx-auto px-4 sm:px-6 lg:px-8 relative z-10">

            {{-- NOTIFIKASI STATUS DOJO --}}
            @if ($role === 'admin_dojo')
                @if (!Auth::user()->dojo_id)
                    <div
                        class="mb-8 bg-rose-950/20 backdrop-blur-md rounded-[2.5rem] p-8 text-white border border-rose-500/30 relative overflow-hidden group">
                        <div
                ...
                @else
                    <div
                        class="mb-8 bg-slate-900/80 backdrop-blur-md rounded-[2rem] p-6 border border-slate-800 shadow-2xl flex items-center justify-between overflow-hidden relative">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                ...
                @foreach ($statCards as $card)
                    <div
                        class="bg-slate-900/60 backdrop-blur-md rounded-[2rem] p-7 border border-slate-800 flex flex-col justify-between hover:border-slate-600 transition-all duration-300 group shadow-lg">
            ...
            {{-- MAIN TABLE SECTION --}}
            <div
                class="bg-slate-900/60 backdrop-blur-md shadow-2xl sm:rounded-[3rem] border border-slate-800 overflow-hidden transition-all">
                <div class="p-6 md:p-10">
                    ...
                    <div class="bg-slate-950/50 rounded-[2rem] p-6 mb-10 border border-slate-800/60 backdrop-blur-xl">
                        <form method="GET" action="{{ route('admin.dashboard') }}"
                            ...
                            <div class="md:col-span-4 flex gap-3">
                                <button type="submit"
                                    class="flex-1 h-12 bg-slate-100 text-slate-900 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-lg active:scale-95">Terapkan
                                    Filter</button>
                    ...
                    <div class="md:hidden space-y-4">
                        @foreach ($members as $member)
                            <div
                                class="bg-slate-950/60 backdrop-blur-md rounded-[2rem] p-6 border border-slate-800 shadow-lg">
